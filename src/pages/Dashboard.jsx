...
const Dashboard = () => {
  ...
  // ── NEW: controls whether we show type-cards or the detail table ──────────
  const [selectedEventType, setSelectedEventType] = useState(null);

  // Update targetLocation as needed

  // ── MOCK DATA (from BillionEyes_V1_Incident.json) ─────────────────────────
  // Remove this block and uncomment the real fetch below when done testing.
  const MOCK_INCIDENTS = [
    {
      ...
      status: "open", // ← change to "Assigned" / "closed" to test other tabs
      allIncidents: [
        {
          image_url:
            "http://192.168.192.177:9000/billion-eyes-images/2026/I-20260202-001.jpg",
    ...
    },
    {
      ...
      description: "Human healthcare services",
    ...
    },
    {
      ...
      status: "closed",
      allIncidents: [
        // ← two bounding boxes → carousel will have 2 slides
        {
          image_url:
            "http://192.168.192.177:9000/billion-eyes-images/2026/I-20260202-007.jpg",
          boundingBoxes: [[119.87, 199.53, 720, 681.89]],
  ...
  useEffect(() => {
    // ── MOCK: load test data instantly ──────────────────────────────────────
    setDashboardData(MOCK_INCIDENTS);
    setAssignedAgency("City Maintenance");
  ...
  // ── filters by active tab AND (if a type is selected) by that type ───────
  ...
  // ── derive unique event types + counts for the cards ─────────────────────
  const eventTypeSummary = (() => {
    const map = {};
    dashboardData.forEach((event) => {
      const type = event.description || "Unknown";
      if (!map[type]) {
        map[type] = { total: 0, open: 0, assigned: 0, closed: 0 };
      }
      map[type].total += 1;
  ...
  // ── renders the event-type cards grid (shown when no type is selected) ────
      ...
      {eventTypeSummary.length === 0 ? (
        ...
        <div className="event-type-cards-grid">
          {eventTypeSummary.map((item, idx) => (
            ...
            >
              {/* top accent bar colour-coded by status mix */}
  ...
  // ── renders the detail table (shown after a type is clicked) ─────────────
        ...
        >
          ...
          >
            <div className="container">
              <div className="row">
                <div className="col-md-12">
                  {/* Conditionally render cards OR detail table */}
          ...
          {/* ── Map Section (Right 30%) – UNTOUCHED ──────────────────────── */}
          <section
            style={{
              width: "30%",
              height: "75%",
            ...
            <div style={{ position: "relative", display: "inline-block" }}>
              {(() => {
                ...
                if (!event) {
                  ...
                  return <p>Error loading image data.</p>;
                }
